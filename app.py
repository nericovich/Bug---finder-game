...
3.  **buggy_code**: Код функции на Python, который содержит **обязательную** и **неочевидную** логическую ошибку. Код не должен содержать синтаксических ошибок.
...
def query_local_model(prompt):
    ...
    try:
        ...
        raise ConnectionError("Не удалось подключиться к Ollama. Убедитесь, что Ollama запущен по адресу http://127.0.0.1:11434.")
...
def get_task():
    """
    Генерирует задачу и выполняет двойную проверку, чтобы убедиться,
    что сгенерированный код действительно содержит ошибку.
    """
    for attempt in range(5):
        try:
            print(f"Попытка №{attempt + 1}: Генерация задачи...")
            task_data = query_local_model(TASK_GENERATION_PROMPT)

            if not all(k in task_data for k in ['task', 'buggy_code', 'title']):
                print("Сгенерированные данные неполные, повторная попытка...")
                continue

            print("Двойная проверка: Анализ сгенерированного кода на наличие ошибки...")
            ...
            if not review_data.get('is_correct'):
                ...
                print("Задача успешно сгенерирована и готова.")
    ...
    return jsonify({"error": "Не удалось сгенерировать качественную задачу с ошибкой после нескольких попыток. Попробуйте снова."}), 500
